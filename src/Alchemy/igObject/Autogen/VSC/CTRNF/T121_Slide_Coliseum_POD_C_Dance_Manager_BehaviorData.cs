namespace Alchemy
{
    [ObjectAttr(ctr: 88, align: 4, metaType: typeof(CVscComponentData))]
    public class T121_Slide_Coliseum_POD_C_Dance_Manager_BehaviorData : CVscComponentData
    {
        [FieldAttr(ctr: 32)] public igHandleMetaField _Entity_0x20 = new();
        [FieldAttr(ctr: 40)] public igHandleMetaField _Entity_0x28 = new();
        [FieldAttr(ctr: 48)] public igHandleMetaField _Vfx_Effect_0x30 = new();
        [FieldAttr(ctr: 56)] public igHandleMetaField _Vfx_Effect_0x38 = new();
        [FieldAttr(ctr: 64)] public igHandleMetaField _Vfx_Effect_id_hpc5i9ij001 = new();
        [FieldAttr(ctr: 72)] public igHandleMetaField _Bolt_Point_id_6giscbvc001 = new();
        [FieldAttr(ctr: 80)] public igHandleMetaField _Vfx_Effect_id_tdq5vset001 = new();
    }
}
